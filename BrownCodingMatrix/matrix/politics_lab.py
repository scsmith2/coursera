...
def create_voting_dict(strlist):
    ...
    """
    poldict = {x.split()[0]:[int(y) for y in x.split()[3:]] for x in strlist}
    return poldict
...
def policy_compare(sen_a, sen_b, voting_dict):
    ...
    out = sum([voting_dict[sen_a][i]*voting_dict[sen_b][i] for i in range(len(voting_dict[sen_a]))])
    return out
...
def most_similar(sen, voting_dict):
    ...
    """
    likemind = 'None'
    agreement = -1*len(voting_dict[sen])-1
    for i in voting_dict:
        if sen != i:
            if policy_compare(sen,i,voting_dict) > agreement:
                agreement = policy_compare(sen,i,voting_dict)
                likemind = i
    return likemind
...
def least_similar(sen, voting_dict):
    ...
    """
    likemind = 'None'
    agreement = len(voting_dict[sen])+1
    for i in voting_dict:
        if sen != i:
            if policy_compare(sen,i,voting_dict) < agreement:
                agreement = policy_compare(sen,i,voting_dict)
...
def find_average_similarity(sen, sen_set, voting_dict):
    ...
    for i in sen_set:
        if i != sen:
            sum = sum + policy_compare(sen,i,voting_dict)
    out = sum / len(sen_set)
    return out


def most_average_democrat():
    #poldict = {x.split()[0]:[int(y) for y in x.split()[3:]] for x in strlist}
    demsens = {x.split()[0] for x in mylist if x.split()[1] == 'D'}
    sens = create_voting_dict(mylist)
    average = 0
    most_average_senator = 'None'
    for i in demsens:
        compsens = sens.copy()
        compsens.pop(i)
        ave = find_average_similarity(i,compsens,sens)
        if ave > average:
            most_average_senator = i
    return most_average_senator
        
    
most_average_Democrat = most_average_democrat()
